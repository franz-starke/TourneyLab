...
<template>
	<RouterLink :to="gameRoute">
		<div class="flex flex-col w-full justify-center items-center">
			...
			<div class="flex flex-row justify-between items-center w-full px-4 py-2 bg-[var(--color-element)] rounded-3xl"
				v-else>
				<div class="flex flex-col">
					<div>
						<p class="text-lg font-bold">{{ $t('games.team') }} {{ team1 }} vs. {{ $t('games.team') }} {{
							team2 }}</p>
					</div>

					<p class="text-sm font-bold text-gray-500">{{ $t('games.ref') }}: {{ $t('games.team') }} {{ referee
					}}</p>
				</div>
				...
				<div
					...
					<input class="flex w-8 text-center text-2xl font-bold" ref="numberInput1" min="0" type="number"
						v-model.number="points[0]" @click.stop.prevent @click="selectInput1()"
						@focus="selectInput1()" />
					:
					<input class="flex w-8 text-center text-2xl font-bold" ref="numberInput2" min="0" type="number"
						v-model.number="points[1]" @click.stop.prevent @click="selectInput2()"
						@focus="selectInput2()" />
				</div>
			</div>
		</div>
