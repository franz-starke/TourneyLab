...
<template>
	<div class="flex flex-col w-full h-full overflow-hidden">
		...
		<div
			class="flex flex-col justify-start items-center gap-4 overflow-y-auto px-4 pt-4 pb-[calc(env(safe-area-inset-bottom)+4rem)]">
			...
			<div class="settingsSection">
				...
				<div class="flex flex-row gap-4">
					<div class="settingsTheme text-[var(--color-text)] bg-[var(--color-element)]"
						:class="{ '!bg-[var(--color-accent)] !text-[var(--color-text-alt)]': settings.appTheme == 'light' }"
						@click="settings.setAppTheme('light')">
						<IconSun />
					</div>
					<div class="settingsTheme text-[var(--color-text)] bg-[var(--color-element)]"
						:class="{ '!bg-[var(--color-accent)]': settings.appTheme == 'dark' }"
						@click="settings.setAppTheme('dark')">
						<IconMoon />
					</div>
				</div>
			</div>
			...
			<div class="settingsSection w-full max-w-100">
				<div class="flex w-full h-18 justify-evenly items-center text-2xl font-bold bg-red-500 text-white rounded-4xl mt-4"
